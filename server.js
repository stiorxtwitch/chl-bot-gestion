...
// ══════════════════════════════════════════════
//  HELPER — Génération de transcript (CHL existant)
// ══════════════════════════════════════════════
async function generateTranscript(channel) {
  ...
  for (const msg of allMessages) {
    ...
    lines.push(`[${date}] ${msg.author.tag}`);
    lines.push(`  ${content}`);
    if (msg.attachments.size > 0) msg.attachments.forEach(att => lines.push(`  📎 Pièce jointe : ${att.url}`));
    if (msg.embeds.length > 0)    msg.embeds.forEach(e => { if (e.title) lines.push(`  📋 Embed : ${e.title}`); });
    lines.push("");
  }
...
const commands = [
  new SlashCommandBuilder()
    ...
    .setDescription("Envoie le panneau de création de ticket CHL dans ce salon")
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption(opt =>
      opt.setName("raison").setDescription("Raison de la suppression").setRequired(true)
    )
    .addStringOption(opt =>
      opt.setName("transcript").setDescription("Générer un transcript du ticket ?").setRequired(true)
  ...
  // ══ NOUVEAU — Système de tickets configurable ══
  new SlashCommandBuilder()
    .setName("ticket")
    .setDescription("Système de tickets configurable par serveur")
    .addSubcommand(sub =>
      sub.setName("setup")
    ...
    .addSubcommand(sub =>
      sub.setName("supprimer")
         ...
         .addStringOption(opt =>
           opt.setName("raison").setDescription("Raison de la suppression").setRequired(false)
         )
         .addBooleanOption(opt =>
           opt.setName("transcript").setDescription("Générer un transcript avant suppression ?").setRequired(false)
         )
    ),
...
// ══════════════════════════════════════════════
//  HELPERS TICKETS CHL (système existant)
// ══════════════════════════════════════════════
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  // ── Boutons dans les tickets configurables (fermer / supprimer / transcript) ──
  if (interaction.isButton() && interaction.customId.startsWith("tticket_")) {
    return ts.onTicketButton(interaction, client);
  }

  // ══════════════════════════════════════════════
  //  SYSTÈME CHL EXISTANT
  // ══════════════════════════════════════════════

  // ── SELECT MENU — Créer un ticket CHL ──
  ...
  if (commandName === "ticket_panel") {
    ...
    return interaction.reply({ content: "✅ Panneau ticket CHL envoyé.", ephemeral: true });
  }
  ...
  if (commandName === "ticket") {
    ...
    if (sub === "setup") {
      ...
      // Stocker la session pour que les modals puissent retrouver ce message
...
app.get("/", (req, res) => res.json({ status: "CHL Bot API v3 ✅ — Stock + Tickets + Recrutement + RDV + Tickets Configurables" }));
