// server.js - Render.com — CHL Bot v3.1 (durci anti-doublons)
...
// ══════════════════════════════════════════════
// Empêche un même interaction.id d'être traité 2 fois (retry Discord, double-clic)
const handledInteractions = new Map(); // id -> timestamp
...
// Verrou par clé (ex: userId, channelName) pour empêcher 2 créations parallèles
const locks = new Map();
...
// ══════════════════════════════════════════════
//  GOOGLE SHEETS — credentials robustes
// ══════════════════════════════════════════════
function loadGoogleCreds() {
  // 1) Préféré : base64 (zéro problème d'échappement)
  if (process.env.GOOGLE_CREDS_B64) {
    try {
      const json = Buffer.from(process.env.GOOGLE_CREDS_B64, "base64").toString("utf-8");
      return JSON.parse(json);
    } catch (e) {
      throw new Error("GOOGLE_CREDS_B64 invalide : " + e.message);
    }
  }
  // 2) Fallback : JSON brut, on tente de réparer les \n mal échappés dans private_key
  if (process.env.GOOGLE_CREDS) {
    ...
    try {
      ...
      throw new Error("GOOGLE_CREDS invalide (utilise plutôt GOOGLE_CREDS_B64) : " + e.message);
    }
  }
  throw new Error("Aucune credential Google : définis GOOGLE_CREDS_B64 (recommandé) ou GOOGLE_CREDS");
}
...
// Cherche un salon par nom dans une catégorie, fetch frais (cache parfois vide)
...
// ══════════════════════════════════════════════
//  DISCORD BOT — singleton login (anti double instance)
// ══════════════════════════════════════════════
...
// clientReady (remplace l'event "ready" déprécié)
...
// Évite les crashs silencieux qui faussent le diagnostic
...
// ══════════════════════════════════════════════
//  INTERACTIONS (avec dédup + verrous)
// ══════════════════════════════════════════════
client.on(Events.InteractionCreate, async (interaction) => {
  // Dédup global : 1 interaction = 1 traitement
  ...
  try {
    ...
    if (interaction.isStringSelectMenu() && interaction.customId === "ticket_create") {
      ...
      // Verrou par user+type pour empêcher 2 créations parallèles
...
app.get("/", (_, res) => res.json({ status: "CHL Bot API v3.1 ✅ — Préfixe : //" }));
...
// Anti-doublons /api/log_stock : même user+item+delta+location en moins de 3s = ignoré
...
app.post("/api/log_stock", async (req, res) => {
  ...
  if (last && now - last < 3000) {
    return res.json({ success: false, error: "Requête dupliquée ignorée (moins de 3s)" });
  }
...
// /api/candidature : verrou par user + check salon existant
app.post("/api/candidature", async (req, res) => {
  ...
  return withLock(`candidature:${safeTag}:${hopitalSuffix}`, async () => {
    try {
      ...
      // Anti-doublon : si le salon existe déjà, on ne recrée pas
      const existing = await findChannelByName(guild, chanName, catId);
      if (existing) {
        return res.json({ success: true, channel: existing.id, channelName: existing.name, duplicate: true });
      }
...
// /api/rendezvous : idem, verrou + check salon existant
